...
The maps we have created above are certainly nice, but what if we want to add labels to them. We can use `geom_text_repel()` from the `ggrepel` package. This allows us to add labels based on a variable and place that label using some manual nudging (which you may not need if you have a few spaced out points). In the example below we label 8 water wells in Hennepin county. In reality, we probably don't need the nudges, but this code shows you how to do it should you want to. 

```{r}
library(ggrepel)

joined_hennepin <- joined %>%
  filter(CTY_NAME == "Hennepin") %>%
  mutate(long = unlist(map(geometry,1)),
           lat = unlist(map(geometry,2))) %>%
  head(8) %>%
  mutate(x_nudge = case_when(stationId == '1000022143' ~ .1, 
                             stationId == '1000022168' ~ .2, 
                             stationId == '104747' ~ -.3,
                             stationId == '104841' ~ -.3,
                             stationId == '105193' ~ -0.2,
                             stationId == '119302' ~ -0.1,
                             stationId == '126374' ~ 0.2,
                             stationId == '126439' ~ 0.2, 
                             TRUE ~ 0), 
         y_nudge = case_when(stationId == '1000022143' ~ 0.05, 
                             stationId == '1000022168' ~ 0, 
                             stationId == '104747' ~ -0.2,
                             stationId == '104841' ~ -0.2,
                             stationId == '105193' ~ 0.15,
                             stationId == '119302' ~ 0,
                             stationId == '126374' ~ 0.15,
                             stationId == '126439' ~ 0.15, 
                             TRUE ~ 0))

ggplot()+
  geom_sf(data=county_boundaries %>% filter(CTY_NAME =="Hennepin"),fill = "cornsilk") +
  geom_sf(data=joined_hennepin)+
  theme_classic()+
  labs(x="", y="")+
  geom_text_repel(data = joined_hennepin,aes(x = long, y = lat, label = stationName), nudge_x = joined_hennepin$x_nudge, nudge_y = joined_hennepin$y_nudge, size =3)+
  theme(axis.line = element_blank(), 
        axis.text = element_blank(),
        axis.ticks = element_blank(),
        legend.position = "bottom", 
