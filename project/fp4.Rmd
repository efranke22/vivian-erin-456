...
This map shows a quick zoom in on Washington County to learn more about the superfund sites testing for PFAS located there. 
...
ggplot(data = pfas_sf, aes(x = `SAMPLE DATE`, fill = DETECT_FLAG)) +
  ...
  labs(fill = "PFAS detected?", title = "PFAS Sample Density 2004 - 2022") +
  theme_classic() +
...
pfas_superfund <- pfas7 %>%
  ...
  mutate(detected = ifelse(DETECT_FLAG == "Y", TRUE, FALSE), year = lubridate::year(`SAMPLE DATE`)) %>%
