...
counties_cropped <- st_crop(counties, st_bbox(well))

ggplot()+
  geom_sf(data = counties_cropped, color = "navajowhite", fill = "ivory", size = 0.5)+
  geom_sf(data= rivers, fill = "blue", color = "blue", alpha=0.3)+
  geom_sf(data = well, size= 1, aes(color = DETECT_FLAG))

ggplot()+
  geom_sf(data = counties_cropped, color = "navajowhite", fill = "ivory", size = 0.5)+
  geom_sf(data= rivers, fill = "blue", color = "blue", alpha=0.3)+
  geom_sf(data = well %>% filter(DETECT_FLAG = "Y"), size= 1, aes(color = commonName))
...
library(lubridate)
superfund_washington %>%
  mutate(ANALYSIS_DATE= str_trim(ANALYSIS_DATE)) %>%
  mutate(year = substr(ANALYSIS_DATE, -4, -1)) %>%
  count(year)
