...
About 552845 samples did not have detection and 155413 did
...
```{r}
superfund %>%
  filter(RESULT_NUMERIC > MIN_ACTION_LEVEL)
```

```{r}
superfund_locY <- superfund_loc %>% filter(DETECT_FLAG == "Y")
grd <- st_make_grid(superfund_locY, square = FALSE, flat_topped = TRUE)
plot(grd)
plot(superfund_locY, add=TRUE)
grd <- st_sf(grd)
grd$n <- lengths(st_intersects(grd, superfund_locY)) 
index <- which(lengths(st_intersects(grd, superfund_locY)) >0)
plot(grd[index])
plot(superfund_locY, add=TRUE)
fnet <- grd[index] %>%
  st_as_sf() %>%
...
points <- st_join(superfund_locY, fnet)
points_count <- points %>%
  st_drop_geometry() %>%
  group_by(id) %>%
  count()
...
superfund %>%
  filter(!is.na(MIN_ACTION_LEVEL)) %>%
  count(ACTION_LEVEL_UNIT)
  filter(RESULT_UNIT != ACTION_LEVEL_UNIT)
  filter(!is.na(MIN_ACTION_LEVEL), RESULT_NUMERIC > MIN_ACTION_LEVEL) %>%
  count(ANALYTE_NAME) %>%
  arrange(desc(n))
```
